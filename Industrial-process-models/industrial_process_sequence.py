...
try:
    ...
    for eaf in range(len(eaf_all_start_times)):
        for other_eaf in range(eaf + 1, len(eaf_all_start_times)):
            s_eaf = eaf_all_start_times[eaf]
            s_other_eaf = eaf_all_start_times[other_eaf]
            m.addConstr(s_eaf - s_other_eaf <= -1 * VC_DURATION)

    for lf in range(LFS):
        for iteration in range(len(eaf_start_times[lf])):
    ...
    for iteration in range(iterations):
        ...
        s_eaf = eaf_all_start_times[iteration]
        m.addConstr(s_eaf + EAF_DURATION + EAF_LF_TRANSFER_TIME + LF_DURATION + LF_VC_TRANSFER_TIME == s_vc, "prd_s_vc_i%d" % (iteration))
    ...
    for eaf in range(EAFS):
        for iteration in range(1, len(eaf_start_times[eaf])):
    ...
    objective = 0
    for eaf in range(EAFS):
        for iteration in range(len(eaf_start_times[eaf])):
